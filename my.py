...
class table:
    def __init__ (self,dbname):
        ...
        self.field_value={}
        self.fieldsorder=[]
        self.defaults={'s':'','d':0,'f':0,'D':curdate2my(),'t':'00:00:00'}
    ...
    def result2value(self,data,key,decode=True,encode=False,tostr=False):
        if key in self.query_fields:
            r=data[self.query_fields.index(key)]
            if tostr and self.field_list[key] != 's':
                r=str(r)
            if decode and type(r)!=unicode and self.field_list[key]=='s':
                if r==None:
                    r='NULL'
                r=r.decode("utf8")
            elif type(r)==unicode and self.field_list[key]=='s' and encode:
                r=r.encode("utf8")
    ...
    def result2values(self,data,decode=True,encode=False,tostr=False):
        values={}
        for key in self.query_fields:
            values[key]=self.result2value(data,key,decode=decode,encode=encode,tostr=tostr)
        return values
